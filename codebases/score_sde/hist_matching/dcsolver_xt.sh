...
python xt_matching_for_dc.py --config=$CONFIG --pair_npz=$PAIR_NPZ --dc_dir=$DC_DIR --ckp_path=$CKPT_PATH --config.sampling.method=dcsolver --config.sampling.steps=$steps --config.sampling.eps=$EPS
done