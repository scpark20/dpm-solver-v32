...
def main(argv):
    sample(FLAGS.config, FLAGS.ckp_path, FLAGS.pair_npz, FLAGS.dc_dir)

def sample(config, ckp_path, pair_npz, dc_dir):
    ...
    sampling_fn = sampling.get_sampling_fn(config, sde, sampling_shape, inverse_scaler, target_matching=True, dc_dir=dc_dir)
    sampling_fn = functools.partial(sampling_fn, score_model)
    data = np.load(pair_npz)
    N = 10
    traj = torch.tensor(data['hist'][:, :N], device=config.device)
    timesteps = torch.tensor(data['timesteps'])
    print(traj.shape, timesteps.shape)
    sampling_fn(target=(traj, timesteps))
