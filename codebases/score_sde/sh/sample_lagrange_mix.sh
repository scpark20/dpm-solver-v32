...
python sample.py --config=$CONFIG --ckp_path=$CKPT_PATH --sample_folder="lagrange_mix_"$steps --config.sampling.method=lagrange_mix --config.sampling.steps=$steps --config.sampling.eps=$EPS
done