...
SCALE_DIR="/data/score_sde_scale_rbf_mix"
...
python sample.py --config=$CONFIG --ckp_path=$CKPT_PATH --scale_dir=$SCALE_DIR --sample_folder="rbf_mix_optimal_"$steps --config.sampling.method=rbf_mix --config.sampling.steps=$steps --config.sampling.eps=$EPS
done