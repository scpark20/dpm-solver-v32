...
python sample.py --config=$CONFIG --ckp_path=$CKPT_PATH --scale_dir=$SCALE_DIR --sample_folder="rbf_const_grid_optimal_"$steps --config.sampling.method=rbf_const_grid --config.sampling.steps=$steps --config.sampling.eps=$EPS
done