...
python sample.py --config=$CONFIG --ckp_path=$CKPT_PATH --sample_folder="rbf_mix_"$steps --config.sampling.method=rbf_mix --config.sampling.steps=$steps --config.sampling.eps=$EPS
done