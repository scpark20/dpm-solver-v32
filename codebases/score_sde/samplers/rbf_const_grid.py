...
class RBFSolverConstGrid:
    ...
    def sample_by_target_matching(self, x, target,
        ...
        for pindex, log_scale_p in tqdm(enumerate(log_scales)):
            for cindex, log_scale_c in tqdm(enumerate(log_scales)):
                y = self.sample(x, steps, t_start, t_end, order, skip_type, method, lower_order_final, log_scale_p, log_scale_c)
    ...
    def sample(self, x, steps,
        ...
        optimal_log_scale_p, optimal_log_scale_c = self.load_optimal_log_scale(steps, order)
