...
class RBFSolverSPDConst:
    ...
    def sample_by_target_matching(self, x, target,
        ...
        if self.scale_dir is not None:
            ...
            np.savez(save_file,
                     optimal_log_scale=optimal_log_scale,
                     losses=losses)
    ...
    def load_optimal_log_scale(self, steps, order):
        try:
            ...
            log_scale = np.load(load_file)['optimal_log_scale']
        except:
            return None
    ...
    def sample(self, x, steps,
        ...
        with torch.no_grad():
            ...
            for i in range(0, steps):
                ...
                # ===predictor===
                s = log_scale_p if log_scale is None else log_scale
                ...
                # ===corrector===
                s = log_scale_c if log_scale is None else log_scale
                lagrange = (s >= self.log_scale_max)
                beta = 1 / (np.exp(s) * abs(lambdas[i+1] - lambdas[i]))
