...
# Mix Prediction + ECP + Same
class RBFSolverMixECPSame:
    def __init__(
            self,
            model_fn,
            noise_schedule,
    ...
    # def get_loss_by_target_matching(self, target, i, hist, lambdas, tau, p, p_prev, log_scale_p, log_scale_c, loss_type='noise'):
    #     if log_scale_c is not None: # for Predictor and Corrector
    #         if loss_type == 'noise':
    #             # for Predictor
    #             beta_noise = float(1 / (np.exp(log_scale_p) * abs(lambdas[i+1] - lambdas[i])))
    ...
    #             hist = hist[i-p+1:i+1][::-1]
    #             pred = sum([nc*h[0] for nc, h in zip(noise_coeffs, hist)])

    #             # for Corrector
    #             i_prev = i-1
    #             beta_noise = float(1 / (np.exp(log_scale_c) * abs(lambdas[i_prev+1] - lambdas[i_prev])))
    #             lambda_array = torch.flip(lambdas[i_prev-p_prev+1:i_prev+2], dims=[0])
    #             noise_coeffs, _ = self.get_coefficients(lambdas[i_prev], lambdas[i_prev+1], lambda_array, beta_noise, 1, tau)
    #             hist = hist[i_prev-p_prev+1:i_prev+2][::-1]
    #             corr = sum([nc*h[0] for nc, h in zip(noise_coeffs, hist)])

    #             # Loss
    #             integral_noise = self.get_integral_vector(lambdas[i-1], lambdas[i+1], lambdas[:1], 0, tau-1.0)
    #             loss = F.mse_loss(target, (pred + corr) / integral_noise)

    #         else:
    #             # for Predictor
    #             beta_data = float(1 / (np.exp(log_scale_p) * abs(lambdas[i+1] - lambdas[i])))
    #             lambda_array = torch.flip(lambdas[i-p+1:i+1], dims=[0])
    #             _, data_coeffs = self.get_coefficients(lambdas[i], lambdas[i+1], lambda_array, 1, beta_data, tau)
    #             hist = hist[i-p+1:i+1][::-1]
    #             pred = sum([dc*h[1] for dc, h in zip(data_coeffs, hist)])
                
    #             # for Corrector
    #             i_prev = i-1
    #             beta_data = float(1 / (np.exp(log_scale_c) * abs(lambdas[i_prev+1] - lambdas[i_prev])))
    #             lambda_array = torch.flip(lambdas[i_prev-p_prev+1:i_prev+2], dims=[0])
    #             _, data_coeffs = self.get_coefficients(lambdas[i_prev], lambdas[i_prev+1], lambda_array, 1, beta_data, tau)
    #             hist = hist[i_prev-p_prev+1:i_prev+2][::-1]
    #             corr = sum([dc*h[1] for dc, h in zip(data_coeffs, hist)])

    #             # Loss
    #             integral_data = self.get_integral_vector(lambdas[i-1], lambdas[i+1], lambdas[:1], 0, tau)
    #             loss = F.mse_loss(target, (pred + corr) / integral_data)

    #     else: # for Predictor Only
    #         if loss_type == 'noise':
    #             # for Predictor
    #             beta_noise = float(1 / (np.exp(log_scale_p) * abs(lambdas[i+1] - lambdas[i])))
    #             lambda_array = torch.flip(lambdas[i-p+1:i+1], dims=[0])
    ...
    #             loss = F.mse_loss(target, pred / integral_noise)
    #         else:
    #             # for Predictor
    #             beta_data = float(1 / (np.exp(log_scale_p) * abs(lambdas[i+1] - lambdas[i])))
    #             lambda_array = torch.flip(lambdas[i-p+1:i+1], dims=[0])
    ...
    #             loss = F.mse_loss(target, pred / integral_data)

    #     return loss

    def get_loss_by_target_matching(self, target, i, hist, x, sr, nr, lambdas, tau, p, p_prev, log_scale_p, log_scale_c, loss_type='noise'):
        if log_scale_c is not None: # for Predictor and Corrector
            if loss_type == 'noise':
                # for Predictor
                beta_noise = float(1 / (np.exp(log_scale_p) * abs(lambdas[i+1] - lambdas[i])))
                ...
                # Loss
                integral_data = self.get_integral_vector(lambdas[i-1], lambdas[i+1], lambdas[:1], 0, tau)
                loss = F.mse_loss(target, ((sr[i+1]/sr[i-1] - nr[i+1]/nr[i-1])*x + sr[i+1]*(pred + corr)) / (nr[i+1]*integral_data))
            ...
            if loss_type == 'noise':
                ...
                # Loss
                integral_data = self.get_integral_vector(lambdas[i], lambdas[i+1], lambdas[:1], 0, tau)
                loss = F.mse_loss(target, ((sr[i+1]/sr[i] - nr[i+1]/nr[i])*x + sr[i+1]*pred) / (nr[i+1]*integral_data))
            else:
                # for Predictor
                beta_data = float(1 / (np.exp(log_scale_p) * abs(lambdas[i+1] - lambdas[i])))
                lambda_array = torch.flip(lambdas[i-p+1:i+1], dims=[0])
    ...
    def sample_by_target_matching(self, x, target,
        ...
        with torch.no_grad():
            ...
            # Noise-P, Noise-C, Data-P, Data-C
            optimal_log_scales = np.zeros((4, steps))
            noise_loss_line_list = []
            data_loss_line_list = []
            ...
            for i in range(0, steps):
                ...
                if i > 0: # for Predictor and Corrector
                    noise_loss_line = np.full(self.log_scale_num, np.inf)
                    data_loss_line  = np.full(self.log_scale_num, np.inf)
                    for index, log_scale in enumerate(log_scales):
                        noise_loss = self.get_loss_by_target_matching(data_target, i, hist, x, signal_rates, noise_rates, lambdas, taus[i], p, p_prev, log_scale, log_scale, loss_type='noise')
                        data_loss  = self.get_loss_by_target_matching(data_target, i, hist, x, signal_rates, noise_rates, lambdas, taus[i], p, p_prev, log_scale, log_scale, loss_type='data')
                        noise_loss_line[index] = noise_loss.item()
                        data_loss_line[index]  = data_loss.item()
                    noise_min_index = np.argmin(noise_loss_line)
                    data_min_index  = np.argmin(data_loss_line)
                    optimal_log_scales[NOISE_PRED, i] = optimal_log_scales[NOISE_CORR, i-1] = log_scales[noise_min_index]
                    optimal_log_scales[DATA_PRED, i] = optimal_log_scales[DATA_CORR, i-1] = log_scales[data_min_index]
                    noise_loss_line_list.append(noise_loss_line)
                    data_loss_line_list.append(data_loss_line)

                else: # for Predictor Only
                    noise_loss_line = np.full(self.log_scale_num, np.inf)
                    data_loss_line  = np.full(self.log_scale_num, np.inf)
                    for index, log_scale in enumerate(log_scales):
                        noise_loss = self.get_loss_by_target_matching(data_target, i, hist, x, signal_rates, noise_rates, lambdas, taus[i], p, p_prev, log_scale, None, loss_type='noise')
                        data_loss  = self.get_loss_by_target_matching(data_target, i, hist, x, signal_rates, noise_rates, lambdas, taus[i], p, p_prev, log_scale, None, loss_type='data')
                        noise_loss_line[index] = noise_loss.item()
                        data_loss_line[index]  = data_loss.item()
                    noise_min_index = np.argmin(noise_loss_line)
                    data_min_index  = np.argmin(data_loss_line)
                    optimal_log_scales[NOISE_PRED, i] = log_scales[noise_min_index]
                    optimal_log_scales[DATA_PRED, i] = log_scales[data_min_index]
                    noise_loss_line_list.append(noise_loss_line)
                    data_loss_line_list.append(data_loss_line)

                if i > 0:
                    # ===corrector===
                    x = self.get_next_sample(x, i-1, hist, signal_rates, noise_rates, lambdas, taus[i-1], p_prev,
                                             optimal_log_scales[NOISE_CORR, i-1], optimal_log_scales[DATA_CORR, i-1], corrector=True)

                # ===predictor===
                x_pred = self.get_next_sample(x, i, hist, signal_rates, noise_rates, lambdas, taus[i], p,
                                              optimal_log_scales[NOISE_PRED, i], optimal_log_scales[DATA_PRED, i], corrector=False)
                p_prev = p
            x = x_pred
        ...
        if self.scale_dir is not None:
            ...
            np.savez(save_file,
                     ...
                     noise_loss_line_list=noise_loss_line_list,
                     data_loss_line_list=data_loss_line_list,
                     )
    ...
    def sample(self, x, steps,
        ...
        with torch.no_grad():
            ...
            for i in range(0, steps):
                ...
                # ===predictor===
                log_s_noise = log_scale_p if log_scales is None else log_scales[NOISE_PRED, i]
                log_s_data = log_scale_p if log_scales is None else log_scales[DATA_PRED, i]
                ...
                # ===corrector===
                log_s_noise = log_scale_c if log_scales is None else log_scales[NOISE_CORR, i]
                log_s_data = log_scale_c if log_scales is None else log_scales[DATA_CORR, i]
