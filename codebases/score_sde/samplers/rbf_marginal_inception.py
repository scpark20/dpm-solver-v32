...
# ECP-Maginal-Inception
class RBFSolverMarginalInception:
    def __init__(
            self,
            model_fn,
            noise_schedule,
    ...
    def get_loss_by_inception_matching(self, pred, target):
        dims=2048
        block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]
        if self.inception is None:
            self.inception = InceptionV3([block_idx]).to(target.device)
        pred_feature = self.inception(pred)[0]
    ...
    def get_loss_by_target_matching(self, i, x, target, hist, noise_rates, log_scale_p, log_scale_c, lambdas, p, p_prev):
        if log_scale_c is not None:
            ...
            loss = self.get_loss_by_inception_matching(xt, target)
