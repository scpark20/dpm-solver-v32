...
class RBFSolverInceptionLag:
    ...
    def get_loss_by_inception_matching(self, i, target, hist, log_scale, lambdas, p):
        dims=2048
        block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]
        if self.inception is None:
            self.inception = InceptionV3([block_idx]).to(target.device)
        pred_eval = self.predict_eval(i, hist, log_scale, lambdas, p)
        pred_feature = self.inception(pred_eval)[0]
        targ_feature = self.inception(target)[0]
        return F.mse_loss(pred_feature, targ_feature)
        ...
        with torch.no_grad():
            ...
            for i in range(0, steps):
                ...
                for log_scale in tqdm(log_scales):
                    loss = self.get_loss_by_inception_matching(i, target, hist, log_scale, lambdas, p)
