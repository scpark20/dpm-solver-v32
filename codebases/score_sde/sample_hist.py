...
def main(argv):
    sample(FLAGS.config, FLAGS.ckp_path, FLAGS.statistics_dir, FLAGS.eval_folder, FLAGS.sample_folder, FLAGS.dc_dir, FLAGS.return_hist)


def sample(config, ckp_path, statistics_dir, eval_folder="samples", sample_dir="sample", dc_dir=None, return_hist=False):
    ...
    if config.sampling.method == "dpm_solver_v3":
        ...
        sampling_fn = sampling.get_sampling_fn(config, sde, sampling_shape, inverse_scaler, dc_dir=dc_dir, return_hist=return_hist)
    ...
    for r in range(16):
        ...
        if len(ret) == 2:
            samples_raw, n = ret
        if len(ret) == 4:
            samples_raw, intermediates, timesteps, n = ret
        ...
        if len(ret) == 2:
            ...
            np.savez_compressed(os.path.join(this_sample_dir, f"samples_{r}.npz"),
                                ...
                                hist=torch.stack(intermediates, dim=0).cpu(),
                                samples_raw=samples_raw.cpu())
