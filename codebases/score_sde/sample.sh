...
#python sample.py --config=$CONFIG --ckp_path=$CKPT_PATH --eval_folder="/data/score_sde_outputs" --sample_folder="rbf_unipc_"$steps --config.sampling.method=rbf_unipc --config.sampling.steps=$steps --config.sampling.variant=bh1 --config.sampling.eps=$EPS
python sample.py --config=$CONFIG --ckp_path=$CKPT_PATH --eval_folder="/data/score_sde_outputs" --sample_folder="rbf_unipc_bh2_"$steps --config.sampling.method=rbf_unipc --config.sampling.steps=$steps --config.sampling.variant=bh2 --config.sampling.eps=$EPS
