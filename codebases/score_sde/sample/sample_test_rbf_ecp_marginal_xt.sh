...
CUDA_VISIBLE_DEVICES='0' python sample_test.py --config=$CONFIG --return_hist=True --ckp_path=$CKPT_PATH --scale_dir=$SCALE_DIR --sample_folder=${METHOD}"_"$steps --config.sampling.method=${METHOD} --config.sampling.steps=$steps --config.sampling.eps=$EPS
done