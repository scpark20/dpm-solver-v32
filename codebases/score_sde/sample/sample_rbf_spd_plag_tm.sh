...
SCALE_DIR="/data/score_sde_scale_rbf_spd_plag_tm"
...
python sample_test.py --config=$CONFIG --ckp_path=$CKPT_PATH --scale_dir=$SCALE_DIR --sample_folder="rbf_spd_plag_tm_"$steps --config.sampling.method=rbf_spd_plag_tm --config.sampling.steps=$steps --config.sampling.eps=$EPS
done