...
CUDA_VISIBLE_DEVICES='2' python sample.py --config=$CONFIG --ckp_path=$CKPT_PATH --scale_dir=$SCALE_DIR --sample_folder=${METHOD}"_"$steps --config.sampling.method=${METHOD} --config.sampling.steps=$steps --config.sampling.eps=$EPS
done