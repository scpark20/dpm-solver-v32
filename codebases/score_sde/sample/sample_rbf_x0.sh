...
python sample.py --config=$CONFIG --ckp_path=$CKPT_PATH --scale_dir=$SCALE_DIR --sample_folder="rbf_x0_"$steps --config.sampling.method=rbf_x0 --config.sampling.steps=$steps --config.sampling.eps=$EPS
done