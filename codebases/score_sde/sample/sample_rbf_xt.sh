...
SCALE_DIR="/data/score_sde_scale_rbf_xt"
...
python sample_test.py --config=$CONFIG --ckp_path=$CKPT_PATH --scale_dir=$SCALE_DIR --sample_folder="rbf_xt_"$steps --config.sampling.method=rbf_xt --config.sampling.steps=$steps --config.sampling.eps=$EPS
done