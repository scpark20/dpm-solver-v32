...
SCALE_DIR="/data/score_sde_scale_rbf_marginal_gram"
...
python sample.py --config=$CONFIG --ckp_path=$CKPT_PATH --scale_dir=$SCALE_DIR --sample_folder="rbf_marginal_gram_"$steps --config.sampling.method=rbf_marginal_gram --config.sampling.steps=$steps --config.sampling.eps=$EPS
done