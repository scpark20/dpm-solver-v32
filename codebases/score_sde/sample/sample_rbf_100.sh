...
SCALE_DIR="/data/score_sde_scale_rbf_100"
...
python sample.py --config=$CONFIG --ckp_path=$CKPT_PATH --scale_dir=$SCALE_DIR --sample_folder="rbf_100_"$steps --config.sampling.method=rbf_100 --config.sampling.steps=$steps --config.sampling.eps=$EPS
