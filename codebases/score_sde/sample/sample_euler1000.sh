...
python sample_test.py --config=$CONFIG --return_prior=True --ckp_path=$CKPT_PATH --eval_folder="/data/score_sde_outputs" --sample_folder="euler_"$steps --config.sampling.method=lagrange --config.sampling.steps=$steps --config.sampling.eps=$EPS
done