...
SCALE_DIR="/data/score_sde_scale_rbf_mix_ecp_same"
...
python sample.py --config=$CONFIG --ckp_path=$CKPT_PATH --scale_dir=$SCALE_DIR --sample_folder="rbf_mix_ecp_same_"$steps --config.sampling.method=rbf_mix_ecp_same --config.sampling.steps=$steps --config.sampling.eps=$EPS
done