...
CUDA_VISIBLE_DEVICES='4' python sample.py --config=$CONFIG --ckp_path=$CKPT_PATH --sample_folder="UniPC_bh2_"$steps --config.sampling.method=uni_pc --config.sampling.steps=$steps --config.sampling.variant=bh2 --config.sampling.eps=$EPS
