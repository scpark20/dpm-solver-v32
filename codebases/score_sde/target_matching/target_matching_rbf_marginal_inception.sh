...
SCALE_DIR="/data/score_sde_scale_rbf_marginal_inception"
...
python target_matching.py --config=$CONFIG --pair_npz=$PAIR_NPZ --scale_dir=$SCALE_DIR --ckp_path=$CKPT_PATH --config.sampling.method=rbf_marginal_inception --config.sampling.steps=$steps --config.sampling.eps=$EPS
done