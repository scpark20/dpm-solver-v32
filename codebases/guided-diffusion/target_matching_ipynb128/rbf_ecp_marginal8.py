...
classes = []
for i in range(100):
    file = f'samples/256x256_diffusion/unipc_200_scale8.0/images/samples_{i}.npz'
...
noises = torch.tensor(np.concatenate(noises, axis=0))[:128]
datas = torch.tensor(np.concatenate(datas, axis=0))[:128]
classes = torch.tensor(np.concatenate(classes, axis=0))[:128]
