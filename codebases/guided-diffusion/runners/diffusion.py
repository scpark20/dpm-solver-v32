...
class Diffusion(object):
    ...
    def sample_raw(self):
        ...
        print('self.sample_raw_flag :', self.sample_raw_flag)
        self.sample()
    ...
    def sample_image(self, x, model, last=True, classifier=None, base_samples=None, classes=None, target=None, number=None, return_hist=False):
        assert last
        try:
            skip = self.args.skip
        except Exception:
        ...
        if self.args.sample_type == "generalized":
        ...
        elif self.args.sample_type == "dcsolver":
        ...
        elif self.args.sample_type == "unipc_rbf":
            from samplers.uni_pc import NoiseScheduleVP, model_wrapper
            from samplers.unipc_rbf import UniPC_RBF
            ...
            if target is None:
                x = rbf.sample(
                    x,
                    steps=(self.args.timesteps - 1 if self.args.denoise else self.args.timesteps),
                    predictor_order=self.args.predictor_order,
                    corrector_order=self.args.corrector_order,
                    skip_type=self.args.skip_type,
                    lower_order_final=self.args.lower_order_final,
                    denoise_to_zero=self.args.denoise,
                )
