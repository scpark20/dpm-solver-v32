...
class UniPC_RBF:
    ...
    def sample_by_target_matching(self, x, target, steps, t_start, t_end, skip_type='logSNR', predictor_order=3, corrector_order=3, lower_order_final=True, denoise_to_zero=None):
        x = x.clone()
        target = target.clone()
        ...
        with torch.no_grad():
            ...
            for i in range(0, steps):
                ...
                # ===predictor===
                p = min(i+1, steps - i, predictor_order) if lower_order_final else min(i+1, predictor_order)
                optimal_log_scale = 0.0
                ...
                # ===corrector===
                p = min(i+1, steps - i, corrector_order) if lower_order_final else min(i+1, corrector_order)
                corr_losses = []
        ...
        if self.scale_dir is not None:
            save_file = os.path.join(self.scale_dir, f'variant={self.variant},NFE={steps},predictor_order={predictor_order},corrector_order={corrector_order}.npz')
            np.savez(save_file,
    ...
    def load_optimal_log_scales(self, steps, predictor_order, corrector_order):
        try:
            load_file = os.path.join(self.scale_dir, f'variant={self.variant},NFE={steps},predictor_order={predictor_order},corrector_order={corrector_order}.npz')
    ...
    def sample(self, x, steps, t_start=None, t_end=None, predictor_order=3, corrector_order=3, skip_type='logSNR', lower_order_final=True, denoise_to_zero=False, log_scale_p=3.0, log_scale_c=0.0):
        ...
        log_scales = self.load_optimal_log_scales(steps, predictor_order, corrector_order)
