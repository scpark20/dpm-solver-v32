...
# ECP-Maginal
class RBFSolverECPMarginalLagC:
    def __init__(
            self,
            model_fn,
            noise_schedule,
    ...
    def get_loss_by_target_matching(self, i, x, target, hist, noise_rates, log_scale_p, lambdas, p, p_prev):
        if i > 0:
            # for predictor
            ...
            coeffs = self.get_lag_coefficients(lambdas[(i-1)], lambdas[(i-1)+1], lambda_array)
            ...
            for i in range(0, steps):
                ...
                if i > 0: # Grid Search
                    ...
                    for pindex, log_scale_p in enumerate(log_scales):
                        loss = self.get_loss_by_target_matching(i, x, target, hist, noise_rates, log_scale_p, lambdas, p, p_prev)
                        loss_line[pindex] = loss.item()
                    min_index = np.argmin(loss_line)
                    optimal_log_scales[0, i] = log_scales[min_index]
                    optimal_log_scales[1, i-1] = self.log_scale_max
